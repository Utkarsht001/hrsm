...
const BRAND_GRADIENT = { background: "linear-gradient(135deg, #0d9488 0%, #134e4a 100%)" } as const;
  ...
  return (
    <div className="mt-6 mb-8 animate-rise">
      <div className="flex items-center gap-3 mb-2">
        <div className="h-12 w-12 rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg" style={BRAND_GRADIENT}>W</div>
        <div>
...
function LoginForm({ email, password, onEmail, onPassword, onSubmit, loading }: LoginFormProps) {
  return (
...
function demoTestId(u: DemoUser): string {
  return u.email.startsWith("alex") ? "demo-login-onboarding-btn" : `demo-login-${u.role}-btn`;
}

function DemoAccounts({ users, onPick }: { users: DemoUser[]; onPick: (u: DemoUser) => void }) {
...
export default function LoginPage() {
  ...
  useEffect(() => {
    api.get<DemoUser[]>("/api/auth/demo-users").then(setDemoUsers).catch(() => { /* swallowed: shown UI works without demo list */ });
  }, []);
    ...
    <div className="bg-app min-h-screen flex items-center justify-center p-4">
      <div className="mobile-shell flex flex-col py-10 px-6 !pb-10" style={{ minHeight: "100vh" }}>
        ...
        <LoginForm
          ...
          loading={loading}
        />
        <DemoAccounts users={demoUsers} onPick={fillDemo} />
      </div>
    </div>
  );
}
